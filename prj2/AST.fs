...
type CType =
  ...
  | CIntPtr // "int*" in C
  | CBoolPtr // "bool*" in C
  | CIntArr of int // "int[N]" in C
  | CBoolArr of int // "bool[N]" in C
...
type Stmt =
  ...
  | ArrUpdate of LineNo * Identifier * Exp * Exp // Ex) "x[1] = 2;"
...
module Exp =
  let rec toStr exp =
    ...
    | Arr (vname, eIdx) -> sprintf "%s[%s]" vname (toStr eIdx)
...
module Stmt =
  let rec toStr indent stmt =
    ...
    | ArrUpdate (_, x, e1, e2) ->
      sprintf "%s%s[%s] = %s" indent x (Exp.toStr e1) (Exp.toStr e2)
